...
% affiliations
\author{\IEEEauthorblockN{Fergus W. Leahy, Joseph Sventek, Paul Harvey}
...
The modern home is becoming increasingly filled with variety of connected devices, each providing a myriad of different and often overlapping services within the home. More recently, ``smart-appliances'' and the Internet of Things have began to invade our homes, attempting to digitize our already existing ``dumb-appliances'' and objects within the home. 

As these devices enter our homes, bringing with them their own ecosystems, protocols, standards and more, the user is faced with an increasingly difficult burden of managing this network of heterogeneous devices. Due to the sheer number and diversity of these devices, many of which provide overlapping services and functionality, problems arise with respect to how these devices co-operate, as well as how to ensure the user's network and information remains secure against new and unanticipated attack vectors.
...
\begin{itemize}
  ...
  \item Homework Database Cache integration, a Java-based proxy enabling the IoT to communicate with a stream database, publish/subscribe engine and automaton, to provide a closed loop of interaction.
\end{itemize}
...
\subsection{Existing Security} % (fold)
\label{sub:existing_security}

TinySec paragraph \cite{TinySec}

MiniSec paragraph \cite{MiniSec}
